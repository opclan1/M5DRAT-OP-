...
client.on('message', async msg => { // eslint-disable-line
	...
	if (command === `play`) {
		...
		if (!voiceChannel) return msg.channel.send('ÃäÇ ÂÓİ æáßä Úáíß Ãä Êßæä İí ŞäÇÉ ÕæÊíÉ áÊÔÛíá ÇáãæÓíŞì!');
		const permissions = voiceChannel.permissionsFor(msg.client.user);
		if (!permissions.has('CONNECT')) {
			return msg.channel.send('áÇ ÃÓÊØíÚ Ãä ÃÊßáã İí åĞå ÇáŞäÇÉ ÇáÕæÊíÉ¡ ÊÃßÏ ãä Ãä áÏí ÇáÕáÇÍíÇÊ ÇáÇÒãÉ !');
		}
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			return msg.channel.send(` **${playlist.title}** Êã ÇÖÇİÉ ÇáŞÇÆãå!`);
		} else {
			try {
				...
				try {
			        ...
			        .setDescription(`**ÇÎÊÇÑ ÑŞã ÇáãŞØÚ** :
...
client.on('message', message => {
    ...
    if (message.author.id !== '234454368072630283') return message.reply('** åĞÇ ÇáÃãÑ İŞØ áÕÇÍÈ ÇáÈæÊ æ ÔßÑÇğğ **')
    ...
    if (message.author.id !== '315477215620104193') return message.reply('** åĞÇ ÇáÃãÑ İŞØ áÕÇÍÈ ÇáÈæÊ æ ÔßÑÇğğ **')
